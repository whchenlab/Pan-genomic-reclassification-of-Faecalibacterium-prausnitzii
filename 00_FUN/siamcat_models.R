...
my_SMOTE_balance <- function(my_data0,max_index=3){
  ...
  for (i in 1:length(feat_list0)) {
    ...
    if(L_max>ceiling(L_min*max_index)){
      ...
      ##at least 3 times
      dup_size0 <- max(max_index,floor(max(table(Group))/min(table(Group))/2))
      if(dup_size0>L_min){
        dup_size0 <- L_min-1
      }
      newData <- SMOTE(feat_list,Group,dup_size=dup_size0)
...
my_siamcat=function(train_feat,train_meta,method="lasso",num.folds=5,num.resample=3,feature.type = "normalized",do.fs=F,nest_top=NULL){
  ...
  siamcat.train <- siamcat(feat=t(train_feat), meta=as.data.frame(train_meta),
                           label='Group', case='Case',siamcat=0)
...
my_siamcat.sel=function(siamcat,
                        consens.thres=0.5,
                        max.show=20,
                        verbose = 0,method='lasso',feature.type = "normalized"){
  # library(SIAMCAT)
  # library('randomForest')
...
my_result=function(feat,meta,method,label,top=NULL,models=NULL,num.folds=5, num.resample=3,
                   new=NULL,auc.matrix=NULL,feature.type = "normalized",consens.thres=0.5,do.fs=F,nest_top=NULL,only_external=F){
  if(!is.null(new)){
    ...
    if(!is.null(top)){
      for(i in new){
        models_top[[i]]=my_siamcat.sel(siamcat=models_top[[i]]$model_back,
                                       ...
                                       max.show=top,
                                       verbose = 0,method=method,feature.type = feature.type)
      }
  ...
  if(only_external==T){
    project_all <- names(feat)[1]
  }else{
    project_all <- names(feat)
  }

  names(models)=project_all
  names(models_top)=project_all
...
my_auc.heatmap=function(datas,models,save.png=F,excluded=F,excluded_name=''){
  ...
  if(excluded==F){
    ...
    data_df <- data_df %>% 
      mutate(study.train=TR) %>%
      filter(method == models)
    data_df <- subset(data_df,!((!study.train%in%excluded_name)&(study.test%in%excluded_name)))
    g2 <-  data_df%>%
      filter(study.test != study.train) %>% 
      group_by(study.train) %>% 
      summarise(AUROC=mean(AUC)) %>% 
...
my_marker.plot <- function(markers_data=NULL,feat_list=NULL,meta_list=NULL,lda_cutoff=2,
                           nproj_cutoff=1,level='genus',change_name=F,cut=F,order=F){
  ...
  ## plot
  # library(r2d3)
  print( r2d3( data = jsondata, script = "marker_comparison_2.js", d3_version = "5") )
...
my_marker.plot <- function(markers_data=NULL,feat_list=NULL,meta_list=NULL,lda_cutoff=2,nproj_cutoff=1,level='genus',change_name=F,cut=F,order=F){
  ...
  if(is.null(markers_data)){
    ...
    for (i in 1:length(lefse.obj)) {
      ...
      lda <- cbind( lda, rep(names(lefse.obj)[i],times=nrow(lda)))
      colnames(lda) <- c('scientific_name','LDA','pvalue','project_id')
      if (!is.null(lda.plot)){
        lda.plot <- rbind( lda.plot, lda)
      }else{
        lda.plot <- lda
  ...
  ## plot
  # library(r2d3)
  print( r2d3( data = jsondata, script = "00_FUN/marker_comparison_2.js", d3_version = "5") )
  return(lda.plot.standard)
}
