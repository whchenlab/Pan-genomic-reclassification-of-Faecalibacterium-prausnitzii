...
my_result_adj=function(feat,meta,method,label,top=NULL,models=NULL,
                       ...
                       pca_plot=F,imbalance=F,max_index=3,batch_group=F,do.con=T,do.fs=F,nest_top=NULL,only_external=F){
  ...
  if(is_cross==F){
    ...
    results=my_result(feat=feat_list,meta=meta_list,method=method,label=label,top=top,models=models,num.folds=num.folds,num.resample=num.resample,
                      new=new,auc.matrix=auc.matrix,feature.type=feature.type,consens.thres=0.5,do.fs=do.fs,nest_top=nest_top,only_external=only_external)
  }else{
...
my_marker_adj=function(feat_list,meta_list,
                       ...
                       level='genus',change_name=F,do.con=T){
  ...
  if(do.con==T){
    ...
    for (p_t_i in names(confounder)) {
      if(is.null(confounder[[p_t_i]])){
        print(p_t_i)
        feat[[p_t_i]] <- feat_list[[p_t_i]]
        meta[[p_t_i]] <- meta_list[[p_t_i]]
