...
# Import data
data <- data.frame(
  Threshold = c("90 threshold", "80 threshold", "70 threshold", "60 threshold", "50 threshold", "total number"),
  F.longum = c(1938, 1940, 1941, 1941, 1941, 2614),
  F.prausnitzii = c(25, 27, 27, 27, 27, 1776),
  F.duncaniae = c(21, 24, 24, 24, 24, 1448),
  F.faecis = c(29, 33, 33, 33, 33, 1332),
  F.un1 = c(13, 15, 15, 15, 15, 502),
  F.un2 = c(2, 2, 2, 2, 2, 391),
  F.hattorii = c(4, 4, 4, 4, 4, 222),
  F.un3 = c(1, 1, 1, 1, 1, 208),
  F.butyricigenerans = c(1, 110, 110, 110, 110, 134),
  F.un4 = c(0, 0, 0, 0, 0, 67),
  F.wellingii = c(1, 1, 1, 1, 1, 67),
  F.langellae = c(1, 1, 1, 1, 1, 64)
)
partial_data <- data.frame(
  Threshold = c("90 threshold", "80 threshold", "70 threshold", "60 threshold", "50 threshold"),
  F.longum = c(217, 215, 215, 215, 215),
  F.prausnitzii = c(6, 6, 6, 6, 6),
  F.duncaniae = c(4, 4, 4, 4, 4),
  F.faecis = c(15, 15, 15, 15, 15),
  F.un1 = c(2, 2, 3, 3, 3),
  F.un2 = c(0, 0, 0, 0, 0),
  F.hattorii = c(1, 1, 1, 1, 1),
  F.un3 = c(0, 0, 0, 0, 0),
  F.butyricigenerans = c(0, 5, 5, 5, 5),
  F.un4 = c(0, 0, 0, 0, 0),
  F.wellingii = c(2, 2, 2, 2, 2),
  F.langellae = c(0, 0, 0, 0, 0)
)
...
data_long <- data %>%
  ...
  mutate(
    ...
    Threshold = factor(Threshold, levels = c("90 threshold", "80 threshold", "70 threshold", 
                                             "60 threshold", "50 threshold"))
  )

partial_long <- partial_data %>%
  pivot_longer(
    cols = -Threshold,
    names_to = "Species",
    values_to = "Unknown"
...
# Merge data
data_long <- data_long %>%
  left_join(partial_long, by = c("Species", "Threshold")) %>%
  rowwise() %>%
  mutate(
    Total = total_counts[[as.character(Species)]],
    Rest = Total - Yes - Unknown,
    # Count percent
    Yes_pct = Yes / Total,
...
plot_data <- data_long %>%
  select(Species, Threshold, Yes, Unknown, Rest, Yes_pct, Unknown_pct, Rest_pct, Total) %>%
  pivot_longer(
    cols = c(Yes, Unknown, Rest),
    names_to = "Category",
    values_to = "Count"
  ) %>%
  pivot_longer(
    cols = c(Yes_pct, Unknown_pct, Rest_pct),
  ...
  filter(
    (Category == "Yes" & Category_pct == "Yes_pct") |
      ...
      (Category == "Rest" & Category_pct == "Rest_pct")
  ) %>%
  mutate(
    Category = factor(Category, levels = c("Rest", "Unknown", "Yes")),
    # 创建百分比标签 - 只在Count大于0且百分比大于0.01（1%）时显示
    Percentage_label = ifelse(Count > 0 & Percentage > 0.01, 
...
plot_data <- plot_data %>%
  mutate(
    ...
    x_pos = (Species_num - 1) * 6 + Threshold_num
  )

# Color mapping
color_mapping <- c(
  # 90 threshold
  "90 threshold_Rest" = "#F7FCF5",
  "90 threshold_Unknown" = "#C7E9C0",
  "90 threshold_Yes" = "#00441B",
  
  # 80 threshold
  "80 threshold_Rest" = "#F7FCF5",
  "80 threshold_Unknown" = "#C7E9C0",
  "80 threshold_Yes" = "#006D2C",
  
  # 70 threshold
  "70 threshold_Rest" = "#F7FCF5",
  "70 threshold_Unknown" = "#C7E9C0",
  "70 threshold_Yes" = "#238B45",
  
  # 60 threshold
  "60 threshold_Rest" = "#F7FCF5",
  "60 threshold_Unknown" = "#C7E9C0",
  "60 threshold_Yes" = "#41AB5D",
  
  # 50 threshold
  "50 threshold_Rest" = "#F7FCF5",
  "50 threshold_Unknown" = "#C7E9C0",
...
ggsave(filename = "GalNac_blasti_all_v2.pdf", width = 8, height = 6)
